...
class IntSet_07_03(  // 집합의 최대 개수
    ...
    // 집합 s와 같은지 확인합니다.
    fun equalTo(s: IntSet_07_03): Boolean {
    ...
    fun unionOf(s1: IntSet_07_03, s2: IntSet_07_03) {
    ...
    // ------------------------------ 연습7-3 ------------------------------//
    // 집합 s의 부분집합인가요?
    fun isSubsetOf(s: IntSet_07_03): Boolean {
        for (i in 0 until num) {
            var j = 0
            while (j < s.num) {
                if (set[i] == s.set[j]) break
    ...
    // 집합 s의 진부분집합인가요?
    fun isProperSubsetOf(s: IntSet_07_03): Boolean {
