...
class IntSet_07_04(  // 집합의 최대 개수
    ...
    // 집합 s와 같은지 확인합니다.
    fun equalTo(s: IntSet_07_04): Boolean {
    ...
    fun unionOf(s1: IntSet_07_04, s2: IntSet_07_04) {
    ...
    // ------------------------------ 연습7-4 ------------------------------//
    // 집합 s1과 s2의 교집합을 복사
    fun intersectionOf(s1: IntSet_07_04, s2: IntSet_07_04) {
        clear()
        for (i in 0 until s1.num) if (s2.contains(s1.set[i])) add(s1.set[i])
    }

    // 집합 s1과 s2의 차집합을 복사
    fun differenceOf(s1: IntSet_07_04, s2: IntSet_07_04) {
        clear()
        for (i in 0 until s1.num) if (!s2.contains(s1.set[i])) add(s1.set[i])
    } // ------------------------------ 연습7-4 ------------------------------//
}