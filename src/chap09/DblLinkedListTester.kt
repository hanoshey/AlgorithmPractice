import chap09.DoubleLinkedList
import java.util.*

internal object DoubleLinkedListTester {
    ...
    fun main(args: Array<String>) {
        ...
        val list = DoubleLinkedList<Data>() // 리스트를 생성
        do {
            when (SelectMenu().also { menu = it!! }) {
                ...
                Menu.ADD -> {
                    data = Data()
                    data.scanData("선택 노드 바로 뒤에 삽입", Data.NO or Data.NAME)
                    list.add(data)
                }
    ...
    internal enum class Menu  // 생성자(constructor)
        ...
        ADD_FIRST("머리 노드 삽입"), ADD_LAST("꼬리 노드 삽입"), ADD("선택 노드 바로 뒤에 삽입"), RMV_FIRST("머리 노드 삭제"), RMV_LAST("꼬리 노드 삭제"), RMV_CRNT(
            "선택 노드 삭제"
        ),
        CLEAR("전체 노드 삭제"), SEARCH_NO("번호 검색"), SEARCH_NAME("이름 검색"), NEXT("선택 노드를 뒤쪽으로"), PREV("선택 노드를 앞쪽으로"), PRINT_CRNT(
            "선택 노드 표시"
        ),
        DUMP("전체 노드 표시"), TERMINATE("종료");
