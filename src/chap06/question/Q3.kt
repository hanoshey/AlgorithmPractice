                ...
                if (a[j - 1] > a[j]) {
                    ...
                    for (k in a.indices) {
                        if (k == j)//자릿수 하나는 부호가 차지하므로 값을 유지시킨다.
                            print("+%%%dd".format(lengthofNum[k]).format(a[k]))
                        else
                            print("%%%dd".format(lengthofNum[k] + 1).format(a[k]))//한 칸을 띄운다.
                    }
                    println()
                    swap(a, j - 1, j)
                    swap(lengthofNum, j - 1, j)//자릿수 배열도 하나를 빼 준다.
                } else {
                    for (k in a.indices) {
                        if (k == j)
                            print("-%%%dd".format(lengthofNum[k]).format(a[k]))
                        else
                            print("%%%dd".format(lengthofNum[k] + 1).format(a[k]))
                    }
                    println()
                }
                j--
